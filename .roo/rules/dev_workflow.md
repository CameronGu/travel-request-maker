...
2.  **`task-master` CLI (For Users & Fallback)**:
    ...
    - Install globally with `pnpm install -g task-master-ai` or use locally via `pnpm dlx task-master-ai ...`.
...
9.  **Commit Changes (If using Git):**
    ...
    *   Consider if a Changeset is needed according to internal versioning guidelines (previously linked to `changeset.md`). If so, run `pnpm run changeset`, stage the generated file, and amend the commit or create a new one.
